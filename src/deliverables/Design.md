...
While the editor uses LSP positions, internally rust-analyzer uses internal FileId + offset positions. The Adapter code allows seamless conversion between the two rapresentations.
...
The request handler asks `ide::Analysis` a simple question: "Where is the definition?".
`ide::Analysis` provides an answer while hiding parsing, semantic analysis, database queries, and cancellation.
